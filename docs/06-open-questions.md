...
| T6 | **Raised 2026-09-01 while wiring Google sign-in.** There is no test database. Vitest loads `dotenv/config` and `verify:oauth` drives the running dev server, so both fixtures read and write the developer's own dev Postgres. That is how `cook@example.test` came to sit next to a real account, and how 229 `agent_activity` rows accumulated in it. The fixtures clean up after themselves, but a failed run leaves debris in data somebody cares about, and a destructive test cannot be written safely at all. Resolve by giving tests their own database: a second `POSTGRES_DB` in `compose.yaml`, a `TEST_DATABASE_URL` and `TEST_APP_DATABASE_URL` that the Vitest setup and the verify script prefer when set, and a truncate-between-files helper. The two-role split has to be reproduced there, or `tests/rls.test.ts` stops proving anything | Before the dev database holds data worth keeping, which is now |
