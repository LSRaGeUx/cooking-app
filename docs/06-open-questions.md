...
| ~~Q10~~ | **ANSWERED 2026-09-07 by building it.** Raised as: the starter set ships aliases that name a different purchase ("spaghetti" under `Pâtes`, "thym" under `Herbes de Provence`, "baguette" under `Pain`), and dropping them would cost the link, the aisle and the allergen derivation. Answered by separating the two jobs instead of editing the data: an alias resolves text to an ingredient, and only the ingredient's own name is read back onto the grocery list. The loose aliases stay, they keep deriving allergens, and the list still says spaghetti | Done |
